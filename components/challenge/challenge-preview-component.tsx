"use client"

import { useRouter } from 'next/navigation';

import CountdownComponent from "@/components/general/countdown-component"
import { formatDistanceToNow } from 'date-fns';
import Image from 'next/image';
import { Button } from '../ui/button';
import { useEffect, useState,useContext } from 'react';
import {
    // Tooltip,
    TooltipContent,
    TooltipProvider,
    TooltipTrigger,
} from "@/components/ui/tooltip"
import TooltipComponent from '@/components/TooltipComponent';
import axios from 'axios';

import { AppContext } from "@/context/StoryContext"

const ChallengePreviewComponent = () => {
    const { 
        inPreview, challengeTitle,
        uploadedImage, imagePlaceholder,
        challengeDate, challengeTime, challengeDescription,
        challengePrice, challengeCurrencySymbol, 
    } = useContext(AppContext)

    function formatDate(date: string) {
        const dateTimeString = `${date}:00`;
        const datetime = new Date(dateTimeString);
        let formattedDate = "";

        formattedDate = datetime.toLocaleString("en-US", {
            month: "short",
            day: "numeric",
            year: "numeric",
            hour: "numeric",
            minute: "numeric",
            second: "numeric",
        });

        // if (date && time) {
        //     const dateTimeString = `${date}T${time}:00`;
        //     const datetime = new Date(dateTimeString);
        //     formattedDate = datetime.toLocaleString("en-US", {
        //         month: "short",
        //         day: "numeric",
        //         year: "numeric",
        //         hour: "numeric",
        //         minute: "numeric",
        //         second: "numeric",
        //     });
        // } else if (date) {
        //     const datetime = new Date(date);
        //     formattedDate = datetime.toLocaleString("en-US", {
        //         month: "short",
        //         day: "numeric",
        //         year: "numeric",
        //     });
        // } else if (time) {
        //     const datetime = new Date(`1970-01-01T${time}:00`);
        //     formattedDate = datetime.toLocaleString("en-US", {
        //         hour: "numeric",
        //         minute: "numeric",
        //         second: "numeric",
        //     });
        // }
        return formattedDate;
    }
    
    return (
            
        <div className='flex flex-col rounded-xl text-white bg-gray-800 border border-gray-600 h-[500px] xs:w-[90%] sm:w-[90%] md:w-[90%] lg:w-[70%]'>
            
            <img id='preview'
                src={ imagePlaceholder ? imagePlaceholder : 'data:image/png;base64,' }
                className='w-full rounded-t-xl h-1/2 object-cover object-center'                     
                alt="Picture of the image"
            />
            <div className="h-1/2 p-4 flex flex-col justify-between">
                <h2 className='font-semibold text-xl mb-2'>{challengeTitle}</h2>
                {challengeDescription && 
                    <div className='mb-2'>
                        <p className='font-semibold text-xs mb-1'>Description</p>                        
                            <p className='text-xs text-gray-500'>
                                {challengeDescription}
                            </p>
                    </div>
                }
                {(challengeCurrencySymbol && challengePrice) && <p className='text-lg mt-3 mb-1'>Bounty: {challengeCurrencySymbol}{challengePrice}</p>}
                {(challengeDate || challengeTime) && <p className='text-sm mb-2'>Expires: {formatDate(challengeDate)}</p>}
                <p className='text-xs pt-5'>
                    {/* <CountdownComponent date={`${challenge.date} ${challenge.time}`} /> */}
                </p>
                

                {/* <Button className='bg-purple-600 w-full mt-4'>Save</Button> */}

            </div>
        </div>
        
    )
}

export default ChallengePreviewComponent